...
function getClienteList(){
	...
	$("#clientelist").append(table);
    	...
    	for(var row=0; row<result.length; row=row+1){
            ...
            fac = fac+' <td> '+telefono+'</td> <td> '+'<button data-bs-toggle="modal" data-bs-target="#clienteupdate" type="button" onclick="getClienteUpdate('+id+')"  class="btn btn-success">Editar</button> <button type="button" onclick="getEliminarCliente('+id+');location.reload();" class="btn btn-danger">Eliminar</button>'+'</td></tr>';    		
            $("#data").append(fac);
    	}
    });
}
...
function getClienteUpdate(id){
    $.getJSON("http://localhost:8080/Tienda/webresources/clientes/"+id, function(result){
        console.log(result);
        $("#idC").val(result.id);
	$("#nombreC").val(result.nombre);
        $("#apellidoC").val(result.apellido);
        $("#tipodocumentoC").val(result.tipodocumento);
        $("#numdocumentoC").val(result.numdocumento);
        $("#telefonoC").val(result.telefono);
        $("#direccionC").val(result.direccion);
    });
}